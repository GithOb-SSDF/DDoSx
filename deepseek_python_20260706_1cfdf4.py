...
{Fore.RED}║  {Fore.GREEN}║  🤖 MCBOT v3.0 - CHAT EN VIVO + INFO REAL                      ║ {Fore.RED}║
{Fore.RED}║  {Fore.GREEN}║  📡 INFO REAL OBTENIDA CON /list                                ║ {Fore.RED}║
{Fore.RED}║  {Fore.GREEN}║  👥 MUESTRA JUGADORES REALES                                    ║ {Fore.RED}║
{Fore.RED}║  {Fore.GREEN}╚══════════════════════════════════════════════════════════════════╝ {Fore.RED}║
...
class MCBot:
    def __init__(self, ip, puerto, nombre, password):
        ...
        self.info_servidor = {"jugadores": "?", "max_jugadores": "?"}
    ...
    def obtener_info_ping(self):
        """Intenta obtener info por ping (solo nombre y versión)"""
        try:
            ...
            if len(data) > 0:
                ...
                if len(campos) >= 4:
                    self.info_servidor['nombre'] = campos[1] if len(campos) > 1 else 'Desconocido'
                    self.info_servidor['version'] = campos[3] if len(campos) > 3 else 'Desconocida'
                    self.info_servidor['protocolo'] = campos[2] if len(campos) > 2 else 'Desconocido'
                    return True
            return False
        except:
            return False
    ...
    def mostrar_info(self):
        ...
        print(f"{Fore.CYAN}║{Fore.YELLOW}  👥 Jugadores: {self.info_servidor.get('jugadores', '?'):<3}/{self.info_servidor.get('max_jugadores', '?'):<3}{' ' * 47}{Fore.CYAN}║")
        
        api = "Desconocida"
        nombre = self.info_servidor.get('nombre', '').lower()
        if 'pocketmine' in nombre:
            api = "PocketMine-MP"
        elif 'nukkit' in nombre:
            api = "Nukkit"
        elif 'genisys' in nombre:
            api = "Genisys"
        
        print(f"{Fore.CYAN}║{Fore.MAGENTA}  📡 API: {api:<54}{Fore.CYAN}║")
    ...
    def conectar(self):
        try:
            ...
            try:
                ...
                print(f"{Fore.YELLOW}[⚠️]{Fore.RESET} No se pudo unir, modo solo lectura")
                self.conectado = False
                return True
        ...
        while self.running:
            try:
                if self.socket:
                    ...
                    if self.play_activo and time.time() - ultimo_play > 5:
    ...
    def _procesar_texto(self, data):
        try:
            ...
            if tipo == 0:
                ...
                # DETECTAR LISTA DE JUGADORES - INFO REAL
                if "players" in msg.lower() and "online" in msg.lower():
                    self._parsear_lista(msg)
    ...
    def _parsear_lista(self, msg):
        """Parsea la lista de jugadores y actualiza la info REAL"""
        try:
            # Buscar "There are X players online: ..."
            match = re.search(r'There are (\d+) players online: (.*)', msg, re.IGNORECASE)
            if not match:
                # Intentar otro formato: "X players online: ..."
            ...
            if match:
                ...
                # Actualizar info del servidor con datos REALES
                self.info_servidor['jugadores'] = str(count)
                if players_str:
                    self.jugadores_online = [p.strip() for p in players_str.split(',') if p.strip()]
                else:
                    self.jugadores_online = []
                ...
                if self.jugadores_online:
                    for i, p in enumerate(self.jugadores_online, 1):
                        print(f"  {Fore.GREEN}{i}.{Fore.RESET} {Fore.CYAN}{p}{Fore.RESET}")
                else:
                    print(f"  {Fore.YELLOW}No hay jugadores{Fore.RESET}")
                print(f"{Fore.CYAN}═══════════════════════════════════════════════════════════════════════{Fore.RESET}\n")
    ...
    def _procesar_status(self, data):
        try:
            ...
            if status == 0:
                ...
                self.conectado = True
                # Obtener lista de jugadores real al conectarse
                time.sleep(1)
                self.enviar_comando("/list")
            elif status == 1:
...
def main():
    ...
    print(f"{Fore.GREEN}[📋]{Fore.RESET} Escribe {Fore.CYAN}/list{Fore.RESET} para ver jugadores online")
    print(f"{Fore.GREEN}[📋]{Fore.RESET} Escribe {Fore.CYAN}/info{Fore.RESET} para ver info del servidor")
